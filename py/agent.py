...
SYSTEM_ASK_FORM = (
    "Ты умный ассистент по здоровью. Сообщи пользователю, что для него автоматически были загружены базовые параметры и покажи их. Предложи начать работу с этими параметрами. Уточни, верно ли загружены параметры?"
)
SYSTEM_ASK_NEXT = (
