...
SYSTEM_ASK_FORM = (
    "Ты умный ассистент по здоровью. Сообщи пользователю, что для него автоматически были загружены базовые параметры и покажи их. Предложи начать работу с этими параметрами."
)
SYSTEM_ASK_NEXT = (
...
def handle_all(message):
    ...
    if user.interaction_state == "goal":
        ...
        if intent == "коррекция веса":
            ...
            bot.send_message(
                message.chat.id,
                "Пока поддерживается только цель 'коррекция веса'. Пожалуйста, напишите свою цель, если вы хотите скорректировать вес."
            )
        return
